...
void SbusReader::update()
{
  ...
  // Drain all pending frames and keep only the newest state.  BLV-R Modbus
  // traffic can make the main loop slower than the SBUS frame rate; reading a
  // single queued frame per loop replays old stick positions for seconds.
  uint8_t drained = 0;
  while (g_bfs_rx.Read() && drained < 16) {
    ++drained;
