...
static const uint8_t MSG_A2J_SUMMARY       = 0x26;
...
static const uint32_t ARM_CMD_TIMEOUT_MS   = 300;
...
{
  uint8_t state;  // 0 = OFF, non-zero = ON
};
...
{
  uint8_t control_mode;   // active source: CTRL_SRC_JETSON / CTRL_SRC_SBUS
  uint8_t comm_state;
  uint8_t motor_state;
  uint8_t arm_state;
};
