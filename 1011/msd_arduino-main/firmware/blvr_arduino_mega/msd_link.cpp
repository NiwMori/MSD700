...
bool MsdProtocolLink::sendMotorStatus(const protocol::MotorStatus & ms)
{
  return sendRaw(protocol::MSG_A2J_MOTOR_STATUS, &ms, (uint8_t)sizeof(ms));
}

bool MsdProtocolLink::sendSystemStatus(const protocol::SystemStatus & st)
{
  return sendRaw(protocol::MSG_A2J_SYSTEM_STATUS, &st, (uint8_t)sizeof(st));
}
