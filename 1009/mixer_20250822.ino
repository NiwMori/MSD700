...
// --- EMA smoothing setup ---
float fbFiltered = 0.0;               // filtered feedback
const float alpha = 0.15;             // smoothing factor (0.05..0.3 typical)
...
// --- Timing for slowdown pulsing ---
unsigned long lastPulseTime = 0;
bool pulseState = false;

// --- Helper: read smoothed feedback ---
float readFilteredFB(int fbPin) {
    int fbRaw = analogRead(fbPin);
    if (fbFiltered == 0.0) fbFiltered = fbRaw;  // initialize first run
    fbFiltered = alpha * fbRaw + (1.0 - alpha) * fbFiltered;
    return fbFiltered;
}

// --- Actuator control ---
void moveActuator(int pulse, int fbPin, int fwdPin, int revPin) {
    float fb = readFilteredFB(fbPin);           // Smoothed ADC value
    float fbPercent = (fb / 1023.0) * 100.0;    // Convert to percentage
    float fbVoltage = (fb / 1023.0) * 5.0;      // Convert to voltage (0-5V)

    // --- Determine slowdown zone ---
    if (fbPercent >= zone1Enter && fbPercent < zone1Exit) {
        slowdownZone = 1;
    } 
    else if (fbPercent >= zone1Exit && fbPercent < zone2Enter) {
        slowdownZone = 1.5;
    } 
    else if (fbPercent >= zone2Enter && fbPercent <= zone2Exit) {
        slowdownZone = 2;
    } 
    else {
        slowdownZone = 0; // above zone2Exit
    }
...
//アームリフト関数
void lift(int pulse) {
    moveActuator(pulse, liftFBPin, liftFWD, liftREV);
}
